...
export default function NewWorkoutError({
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('New workout page error:', error)
    }
  }, [error])
    ...
    <div className="flex items-center justify-center min-h-[60vh]">
      <Card className="max-w-md w-full bg-slate-900/50 border-slate-800">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-white text-2xl">Failed to start workout</CardTitle>
          <CardDescription className="text-slate-400 mt-2">
            {error.message || 'An unexpected error occurred while starting a new workout.'}
          </CardDescription>
        </CardHeader>
